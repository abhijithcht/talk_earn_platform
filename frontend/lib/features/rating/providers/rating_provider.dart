...
final Provider<RatingRepository> ratingRepositoryProvider = Provider((ref) => RatingRepository());

class RatingNotifier extends Notifier<AsyncValue<void>> {
  @override
  AsyncValue<void> build() => const AsyncData<void>(null);

  Future<void> submitRating({required int ratedUserId, required int score}) async {
    state = const AsyncLoading<void>();
    state = await AsyncValue.guard<void>(() async {
      await ref.read(ratingRepositoryProvider).submitRating(ratedUserId: ratedUserId, score: score);
    });
  }
}
